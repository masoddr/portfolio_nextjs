...
export default {
  content: [
    ...
    // Exclude non-existent files that might cause issues
    '!./src/**/*.mdx.bak',
  ],
  darkMode: 'class',
    ...
    extend: {
      ...
      keyframes: {
        ...
        'drift': {
          ...
          '25%': { transform: 'translate(calc(var(--drift-x) * 0.5), calc(var(--drift-y) * 0.3))', opacity: '0.5' },
          '50%': { transform: 'translate(var(--drift-x), var(--drift-y))', opacity: '0.3' },
          '75%': { transform: 'translate(calc(var(--drift-x) * 0.3), calc(var(--drift-y) * 0.7))', opacity: '0.4' },
